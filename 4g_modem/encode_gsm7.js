...
function encodeMessage(b) {
    ...
    for (var e = 0; e < b.length; e++) {
        ...
        if (0 != a)
            if (56320 <= f && 57343 >= f) {
                c += dec2hex(65536 + (a - 55296 << 10) + (f - 56320));
                a = 0;
                continue
            } else a = 0;
        if (55296 <= f && 56319 >= f) a = f;
        else {
            for (cp = dec2hex(f); 4 > cp.length;) cp = "0" + cp;
            c += cp
        }
    }
    return c
...
console.log(encodeMessage(process.argv[2]));