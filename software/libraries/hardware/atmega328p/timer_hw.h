...
#define _TIMER8_SET_OVF_INT(inst, ...)                 NAME(TIMSK,inst) |= _BV(NAME(TOIE, inst))
//get timer overflow
#define _TIMER8_GET_OVF_INT(inst, ...)                 (NAME(TIMSK,inst) >> _BV(NAME(TOIE, inst))) & 0x1

#define _TIMER16_SET_OVF_INT(inst, ...)                 NAME(TIMSK,inst) |= _BV(NAME(TOIE, inst))
//get timer overflow
#define _TIMER16_GET_OVF_INT(inst, ...)                 (NAME(TIMSK,inst) >> _BV(NAME(TOIE, inst))) & 0x1



//set timer output compare match interrupt
#define _TIMER8_SET_OCMP_INT(inst, ...)            NAME(TIMSK,inst) |= _BV(NAME(OCIE,inst, __VA_ARGS__))
//get timer output compare match interrupt
#define _TIMER8_GET_OCMP_INT(inst, ...)            (NAME(TIMSK,inst) >> _BV(NAME(OCIE,inst, __VA_ARGS__))) & 0x1
//set timer output compare match interrupt
#define _TIMER16_SET_OCMP_INT(inst, ...)           NAME(TIMSK,inst) |= _BV(NAME(OCIE,inst, __VA_ARGS__))
//get timer output compare match interrupt
#define _TIMER16_GET_OCMP_INT(inst, ...)           (NAME(TIMSK,inst) >> _BV(NAME(OCIE,inst, __VA_ARGS__))) & 0x1


//clear timer overflow interrupt flag
#define _TIMER8_CLEAR_OVF_INT_FLAG(inst, ...)           NAME(TIFR,inst) |= _BV(NAME(TOV, inst))
//read timer overflow interrupt flag
#define _TIMER8_GET_OVF_INT_FLAG(inst, ...)             (NAME(TIFR,inst) >> _BV(NAME(TOV, inst))) & 0x1
//clear timer overflow interrupt flag
#define _TIMER16_CLEAR_OVF_INT_FLAG(inst, ...)          NAME(TIFR,inst) |= _BV(NAME(TOV, inst))
//read timer overflow interrupt flag
#define _TIMER16_GET_OVF_INT_FLAG(inst, ...)            (NAME(TIFR,inst) >> _BV(NAME(TOV, inst))) & 0x1



//clear timer output compare match interrupt flag
#define _TIMER8_CLEAR_OCMP_INT_FLAG(inst, ...)     NAME(TIFR,inst) |= _BV(NAME(OCF,inst,__VA_ARGS__))
//clear read output compare match interrupt flag
#define _TIMER8_GET_OCMP_INT_FLAG(inst, ...)       (NAME(TIFR,inst) >> _BV(NAME(OCF,inst,__VA_ARGS__))) & 0x1
//clear timer output compare match interrupt flag
#define _TIMER16_CLEAR_OCMP_INT_FLAG(inst, ...)    NAME(TIFR,inst) |= _BV(NAME(OCF,inst,__VA_ARGS__))
//clear read output compare match interrupt flag
#define _TIMER16_GET_OCMP_INT_FLAG(inst, ...)      (NAME(TIFR,inst) >> _BV(NAME(OCF,inst,__VA_ARGS__))) & 0x1
...
/*
    ...
    3       1       1       Fast PWM
*/            
#define _TIMER8_MODE_NORMAL_MODE(inst, ...)             NAME(TCCR, inst, TIMER_CH_A) &= ~(_BV(NAME(WGM,inst,0)) | _BV(NAME(WGM,inst,1))),\
                                                        NAME(TCCR, inst, TIMER_CH_B) &= ~_BV(NAME(WGM,inst,2)) 

#define _TIMER8_MODE_PWM(inst, ...)                     NAME(TCCR, inst, TIMER_CH_A) = ( NAME(TCCR, inst, TIMER_CH_A) & ~_BV(NAME(WGM,inst,1)) ) | _BV(NAME(WGM,inst,0)),\
                                                        NAME(TCCR, inst, TIMER_CH_B) &= ~_BV(NAME(WGM,inst,2)) 

#define _TIMER8_MODE_CTC(inst, ...)                     NAME(TCCR, inst, TIMER_CH_A) = ( NAME(TCCR, inst, TIMER_CH_A) & ~_BV(NAME(WGM,inst,0)) ) | _BV(NAME(WGM,inst,1)),\
                                                        NAME(TCCR, inst, TIMER_CH_B) &= ~_BV(NAME(WGM,inst,2))

#define _TIMER8_MODE_FAST_PWM(inst, ...)                NAME(TCCR, inst, TIMER_CH_A) |= _BV(NAME(WGM,inst,0)) | _BV(NAME(WGM,inst,1)),\
                                                        NAME(TCCR, inst, TIMER_CH_B) &= ~_BV(NAME(WGM,inst,2))

#define _TIMER8_MODE_PWM2(inst, ...)                    NAME(TCCR, inst, TIMER_CH_A) = ( NAME(TCCR, inst, TIMER_CH_A) & ~_BV(NAME(WGM,inst,1)) ) | _BV(NAME(WGM,inst,0)),\
                                                        NAME(TCCR, inst, TIMER_CH_B) |= _BV(NAME(WGM,inst,2))

#define _TIMER8_MODE_FAST_PWM2(inst, ...)               NAME(TCCR, inst, TIMER_CH_A) |= _BV(NAME(WGM,inst,0)) | _BV(NAME(WGM,inst,1)),\
                                                        NAME(TCCR, inst, TIMER_CH_B) |= _BV(NAME(WGM,inst,2))
...
#define _TIMER8_SET_COUNTER(inst, value, ...)           NAME(TCNT, inst ,__VA_ARGS__) = (unsigned char)value

#define _TIMER8_GET_OUTPUT_CMP(inst, ch, ...)               NAME(OCR, inst,ch ,__VA_ARGS__)
#define _TIMER8_SET_OUTPUT_CMP(inst, value, ch, ...)        NAME(OCR, inst,ch , __VA_ARGS__) = (unsigned char)value
...
#define _TIMER8_CLOCK_DISABLED(inst, ...)         NAME(TCCR, inst, TIMER_CH_B) &= ~(_BV(NAME(CS,inst,0)) | _BV(NAME(CS,inst,1)) | _BV(NAME(CS,inst,2)))
#define _TIMER8_CLOCK_PRE_1(inst, ...)            NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~(_BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,1)))) | _BV(NAME(CS,inst,0))
#define _TIMER8_CLOCK_PRE_8(inst, ...)            NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~(_BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,0)))) | _BV(NAME(CS,inst,1))
#define _TIMER8_CLOCK_PRE_64(inst, ...)           NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~_BV(NAME(CS,inst,2))) | _BV(NAME(CS,inst,0)) | _BV(NAME(CS,inst,1))
#define _TIMER8_CLOCK_PRE_256(inst, ...)          NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~(_BV(NAME(CS,inst,1)) | _BV(NAME(CS,inst,0)))) | _BV(NAME(CS,inst,2))
#define _TIMER8_CLOCK_PRE_1024(inst, ...)         NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~_BV(NAME(CS,inst,1))) | _BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,0))
#define _TIMER8_CLOCK_EXT_T0FE(inst, ...)         NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~_BV(NAME(CS,inst,0))) | _BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,1))
#define _TIMER8_CLOCK_EXT_T0RE(inst, ...)         NAME(TCCR, inst, TIMER_CH_B) |= _BV(NAME(CS,inst,0)) | _BV(NAME(CS,inst,1)) | _BV(NAME(CS,inst,2))

//ASYNC specific prescaler
#define _TIMER8_ASYNC_CLOCK_PRE_1(inst, ...)            NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~(_BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,1)))) | _BV(NAME(CS,inst,0))
#define _TIMER8_ASYNC_CLOCK_PRE_8(inst, ...)            NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~(_BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,0)))) | _BV(NAME(CS,inst,1))
#define _TIMER8_ASYNC_CLOCK_PRE_32(inst, ...)           NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~_BV(NAME(CS,inst,2))) | _BV(NAME(CS,inst,0)) | _BV(NAME(CS,inst,1))
#define _TIMER8_ASYNC_CLOCK_PRE_64(inst, ...)           NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~(_BV(NAME(CS,inst,1)) | _BV(NAME(CS,inst,0)))) | _BV(NAME(CS,inst,2))
#define _TIMER8_ASYNC_CLOCK_PRE_128(inst, ...)          NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~_BV(NAME(CS,inst,1))) | _BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,0))
#define _TIMER8_ASYNC_CLOCK_PRE_256(inst, ...)          NAME(TCCR, inst, TIMER_CH_B) = ( NAME(TCCR, inst, TIMER_CH_B) & ~_BV(NAME(CS,inst,0))) | _BV(NAME(CS,inst,2)) | _BV(NAME(CS,inst,1))
#define _TIMER8_ASYNC_CLOCK_PRE_1024(inst, ...)         NAME(TCCR, inst, TIMER_CH_B) |= _BV(NAME(CS,inst,0)) | _BV(NAME(CS,inst,1)) | _BV(NAME(CS,inst,2))
...
#define _TIMER16_MODE_PWM2(inst, ...)              -1
#define _TIMER16_MODE_CTC(inst, ...)                             -1 
#define _TIMER16_MODE_FAST_PWM(inst, ...)                        -1
#define _TIMER16_MODE_FAST_PWM2(inst, ...)                       -1
//ASYNC specific prescaler
