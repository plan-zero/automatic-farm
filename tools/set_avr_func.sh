...
if [ "$1" == "WSL" ]; then
    ...
    if grep -q "avr_ota_push" ~/.bashrc; then
        ...
        if [[ $out == *"python3 is"* ]]; then
            ...
            echo "    python3 $flash_tool_path -T \$2 -H \$1 -P \$tty_port -B 115200 " >> ~/.bashrc
            echo 'else' >> ~/.bashrc
            echo "    python3 $flash_tool_path -T \$2 -H \$1 -K \$3 -P /dev/\$usb_dev -B 115200 " >> ~/.bashrc
            echo 'fi' >> ~/.bashrc
            echo '}' >> ~/.bashrc
        else
    ...
    if [[ $out == *"avrdude is"* ]]; then
        ...
        if grep -q "avr_ota_push" ~/.bashrc; then
            ...
            if [[ $out == *"python3 is"* ]]; then
                ...
                echo 'if [ -z "$3" ]; then' >> ~/.bashrc
                echo "    python3 $flash_tool_path -T \$2 -H \$1 -P /dev/\$usb_dev -B 115200 " >> ~/.bashrc
                echo 'else' >> ~/.bashrc
                echo "    python3 $flash_tool_path -T \$2 -H \$1 -K \$3 -P /dev/\$usb_dev -B 115200 " >> ~/.bashrc
                echo 'fi' >> ~/.bashrc
                echo '}' >> ~/.bashrc
